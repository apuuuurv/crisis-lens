...
export function Dashboard({ onBackToLanding }: DashboardProps) {
  ...
  const [zones, setZones] = useState<{ zone_center_lat: number; zone_center_lng: number; risk_score: number; incident_count: number }[]>([])
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (isAuth && role === "admin") {
        ...
        try {
          const [fetchedIncidents, fetchedResources, fetchedZones] = await Promise.all([
            apiClient.getIncidents(),
            apiClient.getResources(),
            apiClient.getHeatmapZones()
          ])
  ...
  const activeIncidents = incidents.filter((i: Incident) => i.status === "Active").length
    ...
    try {
      ...
      if (updatedIncident) {
        setIncidents((prev: Incident[]) => 
          prev.map((inc: Incident) => inc.id === incidentId ? updatedIncident as Incident : inc)
        )
      }
    } catch (error) {
