...
export const apiClient = {
  ...
  getHeatmapZones: async () => {
    const { ok, data } = await safeFetch("/ai/heatmap-zones")
    if (!ok || !data?.zones) return []
    return data.zones as { zone_center_lat: number; zone_center_lng: number; risk_score: number; incident_count: number }[]
  },
}
